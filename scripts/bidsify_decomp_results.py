...
bids_raw_data_root = str(Path.home()) + '/Downloads/'
bids_derivative_root = str(Path.home()) + '/Downloads/bids_derivatives/'
non_bids_root = Path(str(Path.home()) + '/Downloads/Grison_SCD')

# Link to the source dataset
source_dataset = bids_dataset(datasetname= datasetname,root=bids_raw_data_root)
source_dataset.read()
source_file_list = source_dataset.list_all_file('_emg.edf')

# Get all decomposition log files
files = list_files(non_bids_root,'log.json')
